...
class MediaStreamTrack extends _index.EventTarget {
  ...
  _registerEvents() {
    ...
    if (this.kind === 'video') {
      (0, _EventEmitter.addListener)(this, 'videoTrackDimensionChanged', ev => {
  ...
  _processVideoTrackDimensionChangedQueue() {
    const eventData = _MediaDevices.videoTrackDimensionChangedEventQueue.get(this.id);
    if (!eventData) {
      return;
    }
    this._setVideoTrackDimensions(eventData.width, eventData.height);
    _MediaDevices.videoTrackDimensionChangedEventQueue.delete(this.id);
  }
  release() {
    if (this.remote) {
      return;
